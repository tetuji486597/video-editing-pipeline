# ep19-likecounts — Why like counts are never exact (approximate counters, eventual consistency)
...
**TWO VARIANTS.** `edl.A.json` = "preparing you for your system design interview" intro
(59.583s); `edl.B.json` = "exploring really cool tech" intro (58.750s). Everything after the
intro is **0.833s EARLIER in B** (B's intro take is shorter). Same shared-composition and
separate-caption-file rules as EP18.

**Beats (7 + title):** HOOK / PROBLEM / BUILD / NEW_COMPONENT / APPROX_OK / TRADEOFF / PAYOFF.
**Formats:** half-panel, TAKEOVER, motion-gfx, transparent, corner, TAKEOVER, full-bleed-photo.

**Script direction preserved** from the user's own notes: traffic-jam gridlock on the single
counter (slot_problem's turn, landing on "bottleneck"), numbers gently converging
(slot_new_component), speed-vs-accuracy slider with the bank pinned to accuracy (slot_tradeoff).

**TRADEOFF -> PAYOFF is contiguous, zero gap, on purpose.** Those two beats butt together and a
0.3s gap would have flashed bare footage on the episode's punchline.

**slot_approx_ok's flip time was corrected against word timings.** The plan said output 43.3;
that falls inside "briefly says", so the chip would have contradicted the audio. The flip lands
at local 3.32, one frame after "a million three" begins (output 44.90).

**DROPPED:** an off-script aside at source 95.30-97.86 ("let me know if you want me to talk more
about this in a future episode") sitting between TRADEOFF and PAYOFF. Restore it there if wanted.

**`slot_hook` is a UI recreation, not a screenshot.** Instagram was not captured: the browser was
logged into the user's personal account and a grab would have pulled their profile, DM badge and
"suggested for you" (real private individuals) into a published video. The recreation is also
the only thing that works -- the beat needs a like counter that MOVES and forks.

**LICENCE: `slot_hook`'s Ronaldo photo is CC BY 2.0 and REQUIRES ATTRIBUTION.** See
`episodes/ATTRIBUTIONS_EP14-19.md`.
