...
**Variants.** A = interview (63.083s), B = cool tech (62.375s, -0.7083s).

**MISSPEAK + CORRECTION GAG.** The presenter says "episode 25" in every intro take (this
is 26). Per Gordon's direction it is corrected ON SCREEN: slot_fix26, a TikTok-style white
sticker (`*EPISODE 26` / `he can't count`) popping exactly on the spoken "25" in both
variants (source spans recorded in each EDL's _misspeak_note).

**Overlays.** hook YouTube-vs-call split half-panel / problem re-send-too-late takeover /
build UDP fire-and-forget lane (constant velocity, ease:none) / jitter buffer-tank
half-panel / robot waveform-gap takeover / ai_angle shimmer-regrow transparent / payoff
split-callback half-panel. The EP26 overlay builder stalled mid-batch; the 5 missing
renders were rendered from its authored index.html files by the main session.

**SFX.** `robot_garble.mp3` -- the script's "actual robot-voice" demo -- synthesized from
the presenter's own "glitch constantly" (acrusher 5-bit + vibrato, peak -9.4dB), cued on
"robot voice". Voice meme tt_bruh on the frozen call tile. drip on the jitter tank.
