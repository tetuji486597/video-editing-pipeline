...
**Variants.** A = interview (54.583s), B = cool tech (54.083s, -0.500s). BOTH open with the
"Ana-anonymity" pronunciation BLOOPER (src 53.53-56.36) before the intro, per the EP7
blooper-precedes-clean-delivery convention; the title sticker holding over the stumble is
the gag.

**Real data everywhere.** slot_scan renders rows of the REAL `range/CBFDA` API response
(1,972 suffixes; the real match `C6008F9CAB40...:2266543`); slot_build types the real SHA-1
of password123; slot_payoff holds the real HIBP result capture.

**AUDIO WAR STORY -- read before remixing.** `tt_hold_up.mp3` decoded to +14.3dBFS (broken
source); its square-wave content made the native AAC encoder overshoot ~3.5dB past ANY
limiter ceiling. EP21-B first shipped +1.4dB clipped from a pre-encode WAV measuring a
clean -2.0dBTP. Fixes: the file is sanitized in sfx_shared (see TIKTOK_SFX_NOTES.md);
final_B uses a custom chain (amix -> 200ms -6dB dip at 51.08-51.32 on a plosive -> +2.16dB
linear gain -> 192kHz-oversampled `alimiter=...:level=false` -> AAC). **ffmpeg alimiter
defaults to level=true which re-normalizes output to full scale -- always pass level=false.**
Final peaks: A -1.64dB, B -1.60dB; loudness matched (-15.2 vs -15.4 LUFS).
check_sfx_audible's 11 B-side flags are false (the limiter chain breaks its fitted-gain
assumption); direct RMS deltas confirm cues present (+8.8dB at the notif cue).
