...
**Variants.** A = interview (45.750s), B = cool tech (44.625s, -1.125s).

**Overlays.** hook transparent (runs simultaneously with the title sticker, rule 10 -- the
sticker fade completes exactly at HOOK start, engineered zero-gap) / problem polling
takeover / build arrow-reversal motion-gfx / post POST-payload takeover / sign half-panel
insetting the REAL Stripe docs handler code (Stripe-Signature / construct_event -- the
episode's real image) / payoff corner callback. VERIFIED badge lands at the ONSET of
"them." (ASR box overruns ~1s -- beats.md carries the warning). The builder's freshness
gate caught a stale pre-fix hook mid-build (rule 9's exact trap) -- re-rendered before
delivery.

**SFX.** Voice meme tt_we_do_not_care on the 204-nothing polling replies; cash register on
the payment push. Post-QC volume bumps: whoosh 0.8, keyboard 1.4, payoff pop 0.75 (first
mix buried them under narration). Finals: A -1.79dB, B -1.72dB peaks.
