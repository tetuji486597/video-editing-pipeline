# ep17-fraud — How banks catch fraud in milliseconds (anomaly detection)
...
**Beats (9):** INTRO / HOOK / PROBLEM / BUILD / NEW_COMPONENT / OUTLIER / FEATURE_STORE /
AI_ANGLE / PAYOFF. 62.583s -- the longest in the batch.
**Formats:** half-panel, TAKEOVER, transparent, full-bleed-photo, half-panel, motion-gfx,
TAKEOVER, photo-card. Six distinct.

**EP17 HAS A RECORDED INTRO.** The HOOK beat does not begin until output 6.875, so the hook
overlay starts at 7.500, NOT the no-intro house 2.800. A previous cut shipped it at 2.800 and
put a DECLINED card on screen during the series intro -- that bug is why
`check_overlay_timing.py` exists.

**`slot_new_component` and `slot_outlier` are deliberate mirrors** -- the same scoring gauge
with opposite outcomes (settles green at s=0.155 / slams past the s=0.72 threshold to BLOCKED).
Keep them consistent if either is rebuilt.

**Bank name is invented (`Quorvale`)** and was checked against real institutions; the icon
carries no wordmark.
