...
**Variants.** A = interview intro (51.667s). **B = COLD OPEN** (46.667s, exactly 5.000s
earlier) -- this source has NO "Exploring Really Cool Tech" take (confirmed by Gordon).
B's title sticker carries the hook text (`AI COULDN'T COUNT / THE R'S IN / "STRAWBERRY"`)
over the hook beat instead of a series title.

**Real data.** OpenAI tokenizer captured live: "I love strawberries" = [40, 3047, 106502];
the question capture shows strawberry splitting st|raw|berry [302, 1618, 19772].
slot_new_component uses 40/3047 verbatim; 1618/19772 adapted to the dramatized
straw|berries split (noted in assets/ATTRIB.md). slot_tokens insets the real screenshot.

**Overlays.** hook chat-UI recreation (argued mouth/chin overlap -- no chest band on this
framing, stated not silent) / problem takeover (letter-crawl vs [UNK] wall) / build slice
motion-gfx / new_component ID card-flip takeover / tokens real-screenshot half-panel /
payoff transparent split. Voice meme: tt_bruh on the wrong "2 R's" answer.
Finals: final_A.mp4 -1.79dB peak, final_B.mp4 -1.75dB, all cues audible.
