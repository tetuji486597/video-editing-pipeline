# ep14-infinitescroll — Why feeds never end (lazy loading, cursor pagination)
...
**Beats (7):** HOOK / PROBLEM / BUILD / NEW_COMPONENT / RECYCLE / AI_ANGLE / PAYOFF. 55.875s.
**Formats:** motion-gfx, TAKEOVER, half-panel, TAKEOVER, transparent, motion-gfx, photo-card.

**Take notes.** The AI-angle line has EIGHT takes; used the sixth (123.29). The other complete
read (141.40) drags -- "here's" is boxed 541ms vs 170ms -- and leaves a 330ms hole after
"angle." where this one runs straight through at 40ms.

**Framing.** The BUILD beat is a tight close-up: chin measured at y=1370-1430, i.e. BELOW the
y=1280 content limit, so there is no chest band at all. Resolved as a right-side column with
30px lateral clearance to the hair.

**slot_new_component act order is deliberate and must not be inverted:** the narration names
the cursor FIRST and the breakage LAST, so the overlay resolves on the broken state (a
duplicated row flagged red), not on the fix.
