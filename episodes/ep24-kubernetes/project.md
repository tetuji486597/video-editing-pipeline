...
**Variants.** A = interview (55.083s), B = cool tech (53.917s, -1.1667s).

**Overlays.** hook 3AM half-panel with the real server-rack photo (reused from EP18) +
zzz subversion / problem chaos motion-gfx / build desired-vs-actual half-panel (stacked
digit spans 8->9->10) / control TAKEOVER -- the hero beat: air-traffic tower dashboard, pod
respawn, node failure reschedule, autoscale up/down, radar sweep via trig/onUpdate (never
GSAP rotation on a line) / ai_angle GPU slot-in transparent band / payoff Captain Kube
photo-card (CNCF, CC-BY-4.0). Only ONE takeover this episode (cap is <=2, no minimum).

**SFX.** Voice meme tt_oh_no_no_no on the 3AM flames (then subverted by silence/zzz);
game-like respawn pop in CONTROL per the script. Finals: A -1.51dB, B -1.77dB peaks,
all cues audible (cleanest audibility run of the batch).
