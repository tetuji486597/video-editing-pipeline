...
**Variants.** A = interview (51.542s), B = cool tech (51.000s, -0.5417s).

**Overlays.** hook deploy-counter transparent under the sticker / problem RELEASE DAY
takeover (riser + lever flash) / ci merge-loop motion-gfx / cd conveyor takeover with the
REAL GitHub Actions run screenshot inset (actions/runner release build, 7 green matrix
jobs) / rollback corner chip / ai_angle diff-sweep transparent / payoff crate-to-steps
half-panel. Coverage runs 77-95% (short beats + the 4.5s floor) -- dense, not stretched;
reported per the skill's informational-coverage rule.

**SFX.** Voice meme tt_sheesh on the deploy counter; buildup riser into the lever pull;
conveyor pops per station + correct_ding on DEPLOY. Finals: A -1.73dB, B -1.48dB peaks,
all 32 cues audible (after the checker's control windows were made cue-free -- its first
run false-flagged 3 cues whose controls contained neighbouring cues).
